...
class DummyService:
    async def get_application_audit_trail_search_after(
        ...
        return (
            [{"id": "1", "timestamp": 1554479231870, "username": "u1"}],
            {"next": '1554479231870, "uuid"', "prev": '1554479230000, "uuid2"'},
        )
...
async def test_controller_reads_search_after_from_json_and_calls_service(monkeypatch):
    ...
    async def fake_service(
        application_id, from_date=None, to_date=None, size=25, search_after=None, search_dir=None
    ):
    ...
    monkeypatch.setattr(
        applications_routes,
        "get_admin_service",
        lambda: type("S", (), {"get_application_audit_trail_search_after": fake_service})(),
    )
...
async def test_controller_handles_service_error_returns_503(monkeypatch):
    ...
    monkeypatch.setattr(
        applications_routes,
        "get_admin_service",
        lambda: type("S", (), {"get_application_audit_trail_search_after": bad_service})(),
    )
    assert hasattr(applications_routes, "application_usage_page")
